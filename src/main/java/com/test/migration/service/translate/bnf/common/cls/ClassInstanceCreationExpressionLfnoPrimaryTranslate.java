...
import com.test.migration.service.translate.replace.ClassInstanceCreationExpressionLfnoPrimaryReplace;
...
public class ClassInstanceCreationExpressionLfnoPrimaryTranslate {
    ...
    public String translateClassInstanceCreationExpression_lfno_primary(ParserRuleContext ctx) {
        if (ctx == null || ctx.getRuleIndex() != Java8Parser.RULE_classInstanceCreationExpression_lfno_primary) {
            Log.error("RULE_classInstanceCreationExpression_lfno_primary error");
            return null;
        }
        ...
        ClassInstanceCreationExpressionLfnoPrimaryReplace replace = new ClassInstanceCreationExpressionLfnoPrimaryReplace();
        //'new' typeArguments? annotation* Identifier ('.' annotation* Identifier)* typeArgumentsOrDiamond? '(' argumentList? ')' classBody
